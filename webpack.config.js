...
module.exports = {
  ...
  mode: mode, // Production compresses the data, not typical in development, Changing to development allows us to then use webpack dev server that enables hot reloading
  ...
  devServer: {
    ...
    static: {
      ...
      directory: path.resolve(__dirname, "build"), //If we are running development mode, we are ignoring the output: {} method and just serve the static path.
  ...
  // TODO: look into modules more (is our app one module? what exactly happens when we exclude node_modules?)
  module: {
    rules: [
      {
